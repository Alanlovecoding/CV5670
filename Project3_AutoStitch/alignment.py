...
def computeHomography(f1, f2, matches, A_out=None):
    ...
    # which is the last column of the Vt
...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    for _ in range(nRANSAC):
        ...
        # s = minimum sample size that lets you fit a model
        # randomly pulls out a minimum set of feature matches
        if m == eTranslate:
            ...
            id1 = match.queryIdx
            id2 = match.trainIdx
            M[0,2] = f2[id2].pt[0] - f1[id1].pt[0]
            M[1,2] = f2[id2].pt[1] - f1[id1].pt[1]
        elif m == eHomography:
            pairs = np.random.randint(len(matches), size = 4)
            n_matches = [matches[k] for k in pairs]
            M = computeHomography(f1, f2, n_matches)
        else:
            raise Exception("Error: Invalid motion model.")

        inlier_indices = getInliers(f1, f2, matches, M, RANSACthresh)

        if len(inlier_indices) > len(current_indices):
            current_indices = inlier_indices

    M = leastSquaresFit(f1, f2, matches, m, current_indices)
    #TODO-BLOCK-END
    #END TODO
    return M
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        p1 = np.array([[f1[id1].pt[0], f1[id1].pt[1], 1]]).T
        p2 = np.array([[f2[id2].pt[0], f2[id2].pt[1], 1]]).T
        p2_t = np.dot(M, p1)
        p2_t[0][0] /= p2_t[2][0]
        p2_t[1][0] /= p2_t[2][0]
        p2_t[2][0] = 1
        distance = (p2 - p2_t) * (p2 - p2_t)
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        #TODO-BLOCK-BEGIN
        _matches = [matches[i] for i in inlier_indices]
